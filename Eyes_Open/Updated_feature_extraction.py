...
# =============================================================================
# MODULE 1: DATA LOADING & PREPROCESSING (for Eyes_open)
# =============================================================================
...
# =============================================================================
# MODULE 3: EXTRACT FEATURES FOR ALL EPOCHS FROM Eyes_open SEGMENTS
# =============================================================================
def extract_features_for_EyesOpen(directory, fs, window_size=2, overlap=0.5):
    """
    Extract features for Eyes_open segments from preprocessed BrainVision files.
    Data is segmented into epochs via a sliding window.
    """
    feature_matrices = []
    ...
    for subject_folder in subject_folders:
        ...
        for run_folder in run_folders:
            ...
            if not preprocessed_path.exists():
                logger.warning("Preprocessed Eyes_open folder missing for %s in %s", subject_code, run_folder)
                continue

            # Look for segmented Eyes_open files (BrainVision format, *.vhdr)
            eyesopen_files = list(preprocessed_path.glob("*.vhdr"))
            if not eyesopen_files:
                logger.warning("No Eyes_open BrainVision files found for %s in %s", subject_code, preprocessed_path)
                continue
            logger.debug("Found %d Eyes_open file(s).", len(eyesopen_files))

            epoch_counter = 0
            for eyes_file in eyesopen_files:
                try:
                    logger.debug("Loading Eyes_open file: %s", eyes_file)
                    raw = mne.io.read_raw_brainvision(str(eyes_file), preload=True)
                    raw = preprocess_raw_data(raw, l_freq=1.0, h_freq=40.0, target_fs=fs)
                    eeg_data = raw.get_data()
                    logger.debug("Loaded data shape: %s", eeg_data.shape)
                except Exception as e:
                    logger.error("Error processing EEG file %s for %s in %s: %s", eyes_file, subject_code, run_folder, e)
                    continue

                # Trim first and last 1 second from the Eyes_open segment
                n_samples_total = eeg_data.shape[1]
                trim_samples = int(fs * 1)
                if n_samples_total < 2 * trim_samples:
                    logger.warning("Segment %s too short after trimming. Skipping.", eyes_file.name)
                    continue
...
# =============================================================================
def save_features_by_class(feature_matrices, filename='eeg_features_EyesOpen.h5'):
    """
    Save extracted feature matrices into an HDF5 file, organized by:
      - Class Label (Subject)
      - Run (e.g., 'Run_1' or 'Run_2')
      - Epoch Number
    Naming conventions now use 'EyesOpen' instead of 'Baseline'.
    """
...
def main():
    ...
    logger.info("Starting Eyes_open feature extraction (hierarchical save)...")
    feature_matrices = extract_features_for_EyesOpen(directory, fs, window_size, overlap)
    ...
    filename_out = "new_chetan_EyesOpen_with_ICA.h5"
