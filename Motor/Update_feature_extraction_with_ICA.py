...
def find_baseline_file(preprocessed_path, subject_code):
    """
    Finds the correct baseline file in the given directory based on multiple naming patterns.
    Expected baseline file: {subject_code}_Motor_Baseline_raw.fif
    """
    baseline_patterns = [
        f"{subject_code}_Motor_Baseline_raw.fif",
        f"{subject_code}_S__Baseline_raw.fif",
        f"{subject_code}_S_Baseline_raw.fif"
    ]
    for pattern in baseline_patterns:
        baseline_file = Path(preprocessed_path) / pattern
        if baseline_file.exists():
            logger.debug("Found baseline file: %s", baseline_file)
            return str(baseline_file)
    possible_files = glob.glob(os.path.join(preprocessed_path, f"{subject_code}*Baseline*.fif"))
    if possible_files:
        logger.debug("Using closest matching baseline file: %s", possible_files[0])
        return possible_files[0]
    logger.warning("Baseline file missing for Subject %s in %s", subject_code, preprocessed_path)
    return None
...
# =============================================================================
# MODULE 3: EXTRACT FEATURES FOR ALL EPOCHS FROM S3 SEGMENTS
# =============================================================================
def extract_features_for_S3(directory, fs, window_size=2, overlap=0.5):
    """
    Extract features for subject S3 from segmented BrainVision files.
    The data is segmented into epochs using a sliding window.
    
    This function searches for files saved as:
      {subject_code}_Motor_S3_seg_*_raw.vhdr
    in the subject's motor folder inside the Preprocessed folder.
    """
    feature_matrices = []
    ...
    for subject_folder in subject_folders:
        ...
        for run_folder in run_folders:
            ...
            preprocessed_path = run_folder / "Preprocessed" / f"{subject_code}_Motor"

            baseline_file_path = find_baseline_file(preprocessed_path, subject_code)
            ...
            # Look for S3 segmented files in the motor folder
            stimulus_folder = preprocessed_path
            pattern = f"{subject_code}_Motor_S3_seg_*_raw.vhdr"
            logger.debug("Searching for segmented files in: %s", stimulus_folder)
            logger.debug("Using glob pattern: %s", pattern)
            eeg_files = list(stimulus_folder.glob(pattern))
            if not eeg_files:
                logger.warning("No segmented BrainVision S3 files found for %s in %s", subject_code, stimulus_folder)
                continue
...
# =============================================================================
def save_features_by_class(feature_matrices, filename='eeg_features_S3.h5'):
    """
...
def main():
    ...
    directory = r"G:\Raw Data\Final_new_data"  # Update this path as needed
    ...
    feature_matrices = extract_features_for_S3(directory, fs, window_size, overlap)
    ...
    filename_out = "all_subject_eeg_features_S3_hierarchical.h5"
