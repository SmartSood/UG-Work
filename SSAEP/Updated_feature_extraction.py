...
def find_baseline_file(subject_output_dir, subject_code):
    """
    Finds the correct baseline file in the given subject-specific directory
    based on multiple naming patterns.
    """
    baseline_patterns = [
    ...
    for pattern in baseline_patterns:
        baseline_file = subject_output_dir / pattern
        if baseline_file.exists():
            logger.debug("Found baseline file: %s", baseline_file)
            return str(baseline_file)
    possible_files = glob.glob(os.path.join(subject_output_dir, f"{subject_code}*Baseline*.fif"))
    if possible_files:
        logger.debug("Using closest matching baseline file: %s", possible_files[0])
        return possible_files[0]
    logger.warning("Baseline file missing for Subject %s in %s", subject_code, subject_output_dir)
    return None
...
# =============================================================================
# MODULE 3: EXTRACT FEATURES FOR ALL EPOCHS
# =============================================================================
def extract_features_for_S10(directory, fs, window_size=2, overlap=0.5):
    """
    Extract features for subject S_6 from segmented BrainVision files.
    The data is segmented into epochs using a sliding window.
    Only segments with stimulus S10 in the segment name are processed.
    """
    feature_matrices = []
    ...
    logger.debug("Found %d subject folder(s) under %s", len(subject_folders), directory)

    for subject_folder in subject_folders:
        logger.debug("Processing subject folder: %s", subject_folder)
        subject_code = subject_folder.name.split('_')[0]
        logger.debug("Inferred subject code: %s", subject_code)
        run_folders = [subject_folder / "Run_1", subject_folder / "Run_2"]

        for run_folder in run_folders:
            if not run_folder.exists():
                logger.warning("%s does not exist. Skipping.", run_folder)
                continue

            run_id = run_folder.name.split("_")[-1]
            preprocessed_path = run_folder / "Preprocessed"
            # Use the subject-specific subfolder for saving (e.g., "02_SSAEP")
            subject_output_dir = preprocessed_path / f"{subject_code}_SSAEP"

            baseline_file_path = find_baseline_file(subject_output_dir, subject_code)
            if baseline_file_path is None:
                logger.warning("Baseline file missing for %s in %s", subject_code, run_folder)
                continue
            try:
                baseline_raw = mne.io.read_raw_fif(baseline_file_path, preload=True)
                baseline_raw = preprocess_raw_data(baseline_raw, l_freq=1.0, h_freq=40.0, target_fs=fs)
                logger.debug("Successfully loaded baseline for subject %s, run %s", subject_code, run_id)
            except Exception as e:
                logger.error("Error processing baseline for %s in %s: %s", subject_code, run_folder, e)
                continue

            # Now search for segmented BrainVision files in the subject_output_dir
            # Only process segments that contain "S10" in the filename
            pattern = f"{subject_code}_S10_segment_*_raw.vhdr"
            logger.debug("Searching for segmented files in: %s", subject_output_dir)
            logger.debug("Using glob pattern: %s", pattern)
            eeg_files = list(subject_output_dir.glob(pattern))
            if not eeg_files:
                logger.warning("No segmented BrainVision files with S10 marker found for %s in %s", subject_code, subject_output_dir)
                continue
            logger.debug("Found %d segmented file(s).", len(eeg_files))

            epoch_counter = 0
            for eeg_file in eeg_files:
                try:
                    logger.debug("Loading segmented file: %s", eeg_file)
                    raw_seg = mne.io.read_raw_brainvision(str(eeg_file), preload=True)
                    raw_seg = preprocess_raw_data(raw_seg, l_freq=1.0, h_freq=40.0, target_fs=fs)
                    eeg_data = raw_seg.get_data()
                    logger.debug("Loaded data shape: %s", eeg_data.shape)
                except Exception as e:
                    logger.error("Error processing EEG file %s for %s in %s: %s", eeg_file, subject_code, run_folder, e)
                    continue
...
# =============================================================================
def remove_failing_features(X, threshold=0):
    """
    Drop columns in X that contain more than 'threshold' NaN or infinite values.
    """
    col_nan_counts = np.isnan(X).sum(axis=0)
    col_inf_counts = np.isinf(X).sum(axis=0)
    bad_cols = [i for i in range(X.shape[1]) if col_nan_counts[i] > threshold or col_inf_counts[i] > threshold]
    logger.info("Detected %d columns with invalid values (NaN/inf). Dropping them...", len(bad_cols))
    X_clean = np.delete(X, bad_cols, axis=1)
    return X_clean, bad_cols

# =============================================================================
# MODULE 5: SAVE FEATURES (HIERARCHICAL)
# =============================================================================
def save_features_by_class(feature_matrices, filename='eeg_features_S10.h5'):
    """
...
def main():
    ...
    logger.info("Starting feature extraction (hierarchical save, no ratio normalization)...")
    feature_matrices = extract_features_for_S10(directory, fs, window_size, overlap)
    ...
    filename_out = "chetan_eeg_features_S10_hierarchical.h5"
